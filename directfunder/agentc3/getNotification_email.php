...
if ($_SESSION['user_group'] == 'Admin')
	$sql_click_email = sprintf("SELECT * from mail_log_info where (send_dt >= '%s' and send_dt <= '%s')",$_SESSION['last_real_logout'],$_SESSION['logout_email_get_time']);		
else
	$sql_click_email = sprintf("SELECT * from mail_log_info where ((from_address like '%s') or (mail_rcvr like '%s')) and (send_dt >= '%s' and send_dt <= '%s')",$_SESSION['google_acc_nm'],$_SESSION['google_acc_nm'],$_SESSION['last_real_logout'],$_SESSION['logout_email_get_time']);			
...
<html>
	...
	<body>
	<form name="frmSearch" method="post" action="getNotification_email.php">
		<div class="container">
			<h1>&nbsp;</h1>
			<div class="row">
