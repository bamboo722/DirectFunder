...
$sql_mg_sel = sprintf("select user_id,tw_number,google_acc_nm from admin_user where user_group='%s' and owner='%s'","Manager",$_SESSION['user_login']);
...
while($sql_mg_rec = mysql_fetch_assoc($sql_mg_res))
{
	...
	$sql_agent_sel = sprintf("select user_id,google_acc_nm,tw_number from admin_user where user_group='%s' and owner='%s'","Agent",$agent_ary[$i]['manager']);
	...
	while($sql_agent_rec = mysql_fetch_assoc($sql_agent_res))
	{
		...
		$tw_phone = $sql_agent_rec['tw_number'];
	
	//	$phone = '+1'.$sql_agent_rec['tw_number'];
		// Call
		...
		// SMS
		$sql_click_sms = "SELECT count(*) as sms from sms_log_info where ((from_phone like ('".$tw_phone."')) or (to_phone like ('".$tw_phone."')) and log_time > '".$from_dt."' and log_time < '".$to_dt."')";
	...
	$sql_sel = sprintf("select user_id,tw_number,google_acc_nm from admin_user where user_group='%s' and owner='%s'",'Agent',$agent_ary[$i]['manager']);
	...
	while($sql_rec = mysql_fetch_assoc($sql_res))
	{
		...
		//$phone = $sql_rec['tw_number'];
		// Call
		$mg_sql_click_call = "SELECT count(*) as calls from call_log_info where ((from_phone like ('".$tw_phone."')) or (to_phone like ('".$tw_phone."')) and log_time > '".$from_dt."' and log_time < '".$to_dt."')";
		...
		// SMS
		$mg_sql_click_sms = "SELECT count(*) as sms from sms_log_info where ((from_phone like ('".$tw_phone."')) or (to_phone like ('".$tw_phone."')) and log_time > '".$from_dt."' and log_time < '".$to_dt."')";
