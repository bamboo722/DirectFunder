...
//information about admin report
//get Total, Average
$agent_ary = array();
$agent_ary['total']	= 0;
$agent_ary['average']	= 0;
...
// information about manager reports

$agent_manager_ary = array();
$mg_top_duration=array();
$mg_bottom_duration =array();
$mg_top_conv_min=array();
$mg_bottom_conv_min =array();
$mg_top_ratio=array();
$mg_bottom_ratio=array();
$mg_approved_card_ary = array();
$mg_best_opp_ary = array();
$mg_best_src_ary = array();
$mg_cd_cnt = array();
$mg_opp_cnt = array();
$mg_src_cnt = array();
$mg_max_row_cnt = array();
$mg_sql_click_email_ary = array();
$mg_sql_click_sms_ary = array();
$mg_sql_click_call_ary = array();
$mg_agent_cnt = array();

// get all managers who are belong to admin
$sql_mg_sel = sprintf("select user_id,google_voice_ph,google_acc_nm from admin_user where user_group='%s' and owner='%s'","Manager",$_SESSION['user_login']);
$sql_mg_res = mysql_query($sql_mg_sel) or die(mysql_error());
while($sql_mg_rec = mysql_fetch_assoc($sql_mg_res))
{
	$agent_ary[$i]['manager']	= $sql_mg_rec['user_id'];
	
	$phone = '+1'.$sql_mg_rec['google_voice_ph'];

	// get statistics for every manager 
	$agent_ary[$i]['newLeads']	= 0;
	$agent_ary[$i]['opened_emails']	= 0;
	$agent_ary[$i]['clickthroughs']	= 0;
	$agent_ary[$i]['retryLeads']	= 0;
	$agent_ary[$i]['followUpCount']= 0;
	$agent_ary[$i]['past_due']	= 0;
	$agent_ary[$i]['delinquent']	= 0;
	$agent_ary[$i]['no_follow_up_date']	= 0;
	$agent_ary[$i]['warmLeads']	= 0;
	$agent_ary[$i]['hotLeads']	= 0;
	$agent_ary[$i]['credit_checks']	= 0;
	$agent_ary[$i]['credit_repairs']	= 0;
		
	$agent_ary[$i]['credit_ready']	= 0;
	$agent_ary[$i]['pre_approveds']	= 0;
	$agent_ary[$i]['doc_sents']= 0;
	$agent_ary[$i]['pending_fundings']	= 0;
	$agent_ary[$i]['fundedLeads']= 0;
	$agent_ary[$i]['thirty_day_funding']= 0;
	$agent_ary[$i]['sixty_day_funding']	= 0;
	$agent_ary[$i]['sixty_ninety_day_fundings']	= 0;
	$agent_ary[$i]['clients']	= 0;
	$agent_ary[$i]['other_opportunity']	= 0;
	$agent_ary[$i]['fee_pending']	= 0;
	$agent_ary[$i]['calls'] = 0;
	$agent_ary[$i]['email'] = 0;
	$agent_ary[$i]['sms'] = 0;
	
	$filterAgent = " and ( agent='" . $agent_ary[$i]['manager'] . "'";
	
	$sql_agent_sel = sprintf("select user_id,google_acc_nm,google_voice_ph from admin_user where user_group='%s' and owner='%s'","Agent",$agent_ary[$i]['manager']);
	$sql_agent_res = mysql_query($sql_agent_sel) or die(mysql_error());
	while($sql_agent_rec = mysql_fetch_assoc($sql_agent_res))
	{
		$filterAgent .= " or agent='".$sql_agent_rec['user_id']."'";
		
		/*$sql_stat_sel = sprintf("select * from agent_log_info where agent='%s' order by log_in desc limit 1",$sql_agent_rec['user_id']);
		$sql_stat_res = mysql_query($sql_stat_sel) or die(mysql_error());
		$sql_stat_rec = mysql_fetch_assoc($sql_stat_res);
		if (isset($sql_stat_rec))
		{
			
			$agent_ary[$i]['newLeads']	+= $sql_stat_rec['newLeads'];
			$agent_ary[$i]['opened_emails']	+= $sql_stat_rec['opened_emails'];
			$agent_ary[$i]['clickthroughs']	+= $sql_stat_rec['clickthroughs'];
			$agent_ary[$i]['retryLeads']	+= $sql_stat_rec['retryLeads'];
			$agent_ary[$i]['followUpCount']	+= $sql_stat_rec['followUpCount'];
			$agent_ary[$i]['past_due']	+= $sql_stat_rec['past_due'];
			$agent_ary[$i]['delinquent']	+= $sql_stat_rec['delinquent'];
			$agent_ary[$i]['no_follow_up_date']	+= $sql_stat_rec['no_follow_up_date'];
			$agent_ary[$i]['warmLeads']	+= $sql_stat_rec['warmLeads'];
			$agent_ary[$i]['hotLeads']	+= $sql_stat_rec['hotLeads'];
			$agent_ary[$i]['credit_checks']	+= $sql_stat_rec['credit_checks'];
			$agent_ary[$i]['credit_repairs']	+= $sql_stat_rec['credit_repairs'];
				
			$agent_ary[$i]['credit_ready']	+= $sql_stat_rec['credit_ready'];
			$agent_ary[$i]['pre_approveds']	+= $sql_stat_rec['pre_approveds'];
			$agent_ary[$i]['doc_sents']	+= $sql_stat_rec['doc_sents'];
			$agent_ary[$i]['pending_fundings']	+= $sql_stat_rec['pending_fundings'];
			$agent_ary[$i]['fundedLeads']	+= $sql_stat_rec['fundedLeads'];
			$agent_ary[$i]['thirty_day_funding']	+= $sql_stat_rec['thirty_day_funding'];
			$agent_ary[$i]['sixty_day_funding']	+= $sql_stat_rec['sixty_day_funding'];
			$agent_ary[$i]['sixty_ninety_day_fundings']	+= $sql_stat_rec['sixty_ninety_day_fundings'];
			$agent_ary[$i]['clients']	+= $sql_stat_rec['clients'];
			$agent_ary[$i]['other_opportunity']	+= $sql_stat_rec['other_opportunity'];
			$agent_ary[$i]['fee_pending']	+= $sql_stat_rec['fee_pending'];
		}*/
		
		$phone = '+1'.$sql_agent_rec['google_voice_ph'];
		// Call
		$sql_click_call = "SELECT count(*) as calls from call_log_info where ((from_phone like ('".$phone."')) or (to_phone like ('".$phone."')) and log_time > '".$from_dt."' and log_time < '".$to_dt."')";
		...
		// SMS
		$sql_click_sms = "SELECT count(*) as sms from sms_log_info where ((from_phone like ('".$phone."')) or (to_phone like ('".$phone."')) and log_time > '".$from_dt."' and log_time < '".$to_dt."')";
		$sql_click_sms_resul = mysql_query($sql_click_sms) or die(mysql_error());
		$sql_click_sms_ary = mysql_fetch_assoc($sql_click_sms_resul);

		// Email
		
		$sql_click_email = "SELECT count(*) as email  from mail_log_info where ((from_address like ('".$sql_agent_rec['google_acc_nm']."')) or (mail_rcvr like ('".$sql_agent_rec['google_acc_nm']."')) and send_dt > '".$from_dt."' and send_dt < '".$to_dt."') ";
	...
	$sql_sel_fees = sprintf("select sum(fee_amount) as fee_amounts from opportunity_info,(select customer_id from customer_info,(select user_id from admin_user where owner='%s') as selected_agent_info where customer_info.agent=selected_agent_info.user_id) as selected_customer_info where opportunity_info.customer_id = selected_customer_info.customer_id  and  date_paid>'%s' and date_paid<'%s'",$agent_ary[$i]['manager'],$from_dt,$to_dt);
	...
	//get Total, Average
	$agent_manager_ary[$i]['total']	= 0;
	$agent_manager_ary[$i]['average']	= 0;
		
	// get all agents who are belong to manager 
	$sql_sel = sprintf("select user_id,google_voice_ph,google_acc_nm from admin_user where user_group='%s' and owner='%s'",'Agent',$agent_ary[$i]['manager']);
	$sql_res = mysql_query($sql_sel) or die(mysql_error());
	$i_mg =0;
	while($sql_rec = mysql_fetch_assoc($sql_res))
	{
		$agent_manager_ary[$i][$i_mg]['agent']	= $sql_rec['user_id'];
		
		$phone = '+1'.$sql_rec['google_voice_ph'];
		// Call
		$mg_sql_click_call = "SELECT count(*) as calls from call_log_info where ((from_phone like ('".$phone."')) or (to_phone like ('".$phone."')) and log_time > '".$from_dt."' and log_time < '".$to_dt."')";
		$mg_sql_click_call_resul = mysql_query($mg_sql_click_call) or die(mysql_error());
		$mg_sql_click_call_ary[$i_msg] = mysql_fetch_assoc($mg_sql_click_call_resul);

		// SMS
		$mg_sql_click_sms = "SELECT count(*) as sms from sms_log_info where ((from_phone like ('".$phone."')) or (to_phone like ('".$phone."')) and log_time > '".$from_dt."' and log_time < '".$to_dt."')";
		$mg_sql_click_sms_resul = mysql_query($mg_sql_click_sms) or die(mysql_error());
		$mg_sql_click_sms_ary[$i_msg] = mysql_fetch_assoc($mg_sql_click_sms_resul);

		// Email
		
		$mg_sql_click_email = "SELECT count(*) as email  from mail_log_info where ((from_address like ('".$sql_rec['google_acc_nm']."')) or (mail_rcvr like ('".$sql_rec['google_acc_nm']."')) and send_dt > '".$from_dt."' and send_dt < '".$to_dt."') ";
		$mg_sql_click_email_resul = mysql_query($mg_sql_click_email) or die(mysql_error());
		$mg_sql_click_email_ary[$i_msg] = mysql_fetch_assoc($mg_sql_click_email_resul);

		// get statistics for every agent 
		$sql_stat_sel = sprintf("SELECT SUM(CASE WHEN priority_opt = 'New' THEN 1 ELSE 0 END) As newLeads,SUM(CASE WHEN priority_opt = 'Retry' THEN 1 ELSE 0 END) As retryLeads,SUM(CASE WHEN (apply_dt = now() ) THEN 1 ELSE 0 END) As today_task, SUM(CASE WHEN (priority_opt = 'Hot' ) THEN 1 ELSE 0 END) As hotLeads,SUM(CASE WHEN priority_opt = 'Warm' THEN 1 ELSE 0 END) As warmLeads,SUM(CASE WHEN priority_opt = 'Credit Check' THEN 1 ELSE 0 END) As credit_checks,SUM(CASE WHEN priority_opt = 'Credit Repair' THEN 1 ELSE 0 END) As credit_repairs,SUM(CASE WHEN priority_opt = 'Credit Ready' THEN 1 ELSE 0 END) As credit_ready,SUM(CASE WHEN priority_opt = 'Doc. Sent' THEN 1 ELSE 0 END) As doc_sents,SUM(CASE WHEN priority_opt = 'Funded' THEN 1 ELSE 0 END) As fundedLeads, SUM(CASE WHEN priority_opt = 'Pending Funding' THEN 1 ELSE 0 END) As pending_fundings, SUM(CASE WHEN priority_opt = 'Fee Pending' THEN 1 ELSE 0 END) As fee_pendings, SUM(CASE WHEN priority_opt = 'Pre-approved' THEN 1 ELSE 0 END) As pre_approveds,SUM(CASE WHEN priority_opt = 'Funded' THEN 1 ELSE 0 END) As fundedLeads, SUM(CASE WHEN priority_opt = 'Clients' THEN 1 ELSE 0 END) As clients,SUM(CASE WHEN priority_opt = 'Fee Pending' THEN 1 ELSE 0 END) As fee_pending,SUM(CASE WHEN priority_opt = 'Clickthroughs' THEN 1 ELSE 0 END) As clickthroughs,SUM(CASE WHEN priority_opt = 'Opened Emails' THEN 1 ELSE 0 END) As opened_emails,SUM(CASE WHEN is_opportunity_yes = 1 THEN 1 ELSE 0 END) As other_opportunity from customer_info WHERE agent='%s'",$agent_manager_ary[$i][$i_mg]['agent']);
		$sql_stat_res = mysql_query($sql_stat_sel) or die(mysql_error());
		$sql_stat_rec = mysql_fetch_assoc($sql_stat_res);
		if (isset($sql_stat_rec))
		{
			
			$agent_manager_ary[$i][$i_mg]['newLeads']	= (int)$sql_stat_rec['newLeads'];
			$agent_manager_ary[$i][$i_mg]['opened_emails']	= (int)$sql_stat_rec['opened_emails'];
			$agent_manager_ary[$i][$i_mg]['clickthroughs']	= (int)$sql_stat_rec['clickthroughs'];
			$agent_manager_ary[$i][$i_mg]['retryLeads']	= (int)$sql_stat_rec['retryLeads'];
			$agent_manager_ary[$i][$i_mg]['followUpCount']	= (int)$sql_stat_rec['followUpCount'];
			$agent_manager_ary[$i][$i_mg]['past_due']	= (int)$sql_stat_rec['past_due'];
			$agent_manager_ary[$i][$i_mg]['delinquent']	= (int)$sql_stat_rec['delinquent'];
			$agent_manager_ary[$i][$i_mg]['no_follow_up_date']	= (int)$sql_stat_rec['no_follow_up_date'];
			$agent_manager_ary[$i][$i_mg]['warmLeads']	= (int)$sql_stat_rec['warmLeads'];
			$agent_manager_ary[$i][$i_mg]['hotLeads']	= (int)$sql_stat_rec['hotLeads'];
			$agent_manager_ary[$i][$i_mg]['credit_checks']	= (int)$sql_stat_rec['credit_checks'];
			$agent_manager_ary[$i][$i_mg]['credit_repairs']	= (int)$sql_stat_rec['credit_repairs'];
			
			
			$agent_manager_ary[$i][$i_mg]['credit_ready']	= (int)$sql_stat_rec['credit_ready'];
			$agent_manager_ary[$i][$i_mg]['pre_approveds']	= (int)$sql_stat_rec['pre_approveds'];
			$agent_manager_ary[$i][$i_mg]['doc_sents']	= (int)$sql_stat_rec['doc_sents'];
			$agent_manager_ary[$i][$i_mg]['pending_fundings']	= (int)$sql_stat_rec['pending_fundings'];
			$agent_manager_ary[$i][$i_mg]['fundedLeads']	= (int)$sql_stat_rec['fundedLeads'];
			$agent_manager_ary[$i][$i_mg]['thirty_day_funding']	= (int)$sql_stat_rec['thirty_day_funding'];
			$agent_manager_ary[$i][$i_mg]['sixty_day_funding']	= (int)$sql_stat_rec['sixty_day_funding'];
			$agent_manager_ary[$i][$i_mg]['sixty_ninety_day_fundings']	= (int)$sql_stat_rec['sixty_ninety_day_fundings'];
			$agent_manager_ary[$i][$i_mg]['clients']	= (int)$sql_stat_rec['clients'];
			$agent_manager_ary[$i][$i_mg]['other_opportunity']	= (int)$sql_stat_rec['other_opportunity'];
			$agent_manager_ary[$i][$i_mg]['fee_pending']	= (int)$sql_stat_rec['fee_pending'];
			
			if (isset($mg_sql_click_call_ary[$i_msg]['calls']) and $mg_sql_click_call_ary[$i_msg]['calls']!="")
				$agent_manager_ary[$i][$i_mg]['calls']	= (int)$mg_sql_click_call_ary[$i_msg]['calls'];
			else
				$agent_manager_ary[$i][$i_mg]['calls']	= 0;
			if (isset($mg_sql_click_email_ary[$i_msg]['email']) and $mg_sql_click_email_ary[$i_msg]['email']!="")
				$agent_manager_ary[$i][$i_mg]['email']	= (int)$mg_sql_click_email_ary[$i_msg]['email'];
			else
				$agent_manager_ary[$i][$i_mg]['email']	= 0;
			if (isset($mg_sql_click_sms_ary[$i_msg]['sms']) and $mg_sql_click_sms_ary[$i_msg]['sms']!="")
				$agent_manager_ary[$i][$i_mg]['sms']	= (int)$mg_sql_click_sms_ary[$i_msg]['sms'];
			else
				$agent_manager_ary[$i][$i_mg]['sms']	= 0;
			
		}
		
		// get performance info for every agent 
		$sql_sum = sprintf("select sum(dur_hour*3600+dur_min*60+dur_sec) as duration,sum(conv_min) as conv_min, sum(call_made) as call_made,sum(call_conn) as call_conn,sum(sms_sent) as sms_sent, sum(sms_recv) as sms_recv,sum(eml_sent) as eml_sent, sum(eml_recv) as eml_recv,sum(ratio) as ratio from agent_log_info where agent='%s' and log_in>'%s' and log_in<'%s'",$agent_manager_ary[$i][$i_mg]['agent'],$from_dt,$to_dt);
		$result_sum = mysql_query($sql_sum) or die(mysql_error());
		$sum_array = mysql_fetch_assoc($result_sum);
		$agent_manager_ary[$i][$i_mg]['duration']	= (int)$sum_array['duration'];
		$agent_manager_ary[$i][$i_mg]['conv_min']	= (int)$sum_array['conv_min'];
		$agent_manager_ary[$i][$i_mg]['call_made']	= (int)$sum_array['call_made'];
		$agent_manager_ary[$i][$i_mg]['call_conn']	= (int)$sum_array['call_conn'];
		$agent_manager_ary[$i][$i_mg]['sms_sent']	= (int)$sum_array['sms_sent'];
		$agent_manager_ary[$i][$i_mg]['sms_recv']	= (int)$sum_array['sms_recv'];
		$agent_manager_ary[$i][$i_mg]['eml_sent']	= (int)$sum_array['eml_sent'];
		$agent_manager_ary[$i][$i_mg]['eml_recv']	= (int)$sum_array['eml_recv'];
		if ((int)($agent_manager_ary[$i][$i_mg]['duration']/60) == 0)
			$agent_manager_ary[$i][$i_mg]['ratio']=0;
		else
			$agent_manager_ary[$i][$i_mg]['ratio']=sprintf("%.1f",(float)($agent_manager_ary[$i][$i_mg]['conv_min']*100/((int)($agent_manager_ary[$i][$i_mg]['duration']/60))));	
		
		// get Revenue, Total, Average
		
		// get sum of fee amounts under opportunity tab of customer info for each agent
		$agent_manager_ary[$i][$i_mg]['revenue'] = 0;
				
		$sql_sel_fees = sprintf("select sum(fee_amount) as fee_amounts from opportunity_info,(select customer_id from customer_info where agent='%s') as selected_customer_info where (opportunity_info.customer_id = selected_customer_info.customer_id) and  date_paid>'%s' and date_paid<'%s'",$agent_manager_ary[$i][$i_mg]['agent'],$from_dt,$to_dt);
		$sql_res_fees = mysql_query($sql_sel_fees) or die(mysql_error());
		$sql_rec_fees = mysql_fetch_assoc($sql_res_fees);
		if (isset($sql_rec_fees) and  $sql_rec_fees['fee_amounts']!=NULL)
			$agent_manager_ary[$i][$i_mg]['revenue'] += $sql_rec_fees['fee_amounts'];		
			
		
		$agent_manager_ary[$i]['total']+=$agent_manager_ary[$i][$i_mg]['revenue'];	
		$i_mg++;
	}
	if ($i_mg != 0)
		$agent_manager_ary[$i]['average'] = round($agent_manager_ary[$i]['total']/$i_mg,2);
	
	
	$mg_agent_cnt[$i] = $i_mg;
	
	$mg_top_duration[$i]=$agent_manager_ary[$i][0]['duration'];
	$mg_bottom_duration[$i] =$agent_manager_ary[$i][0]['duration'];
	$mg_top_conv_min[$i]=$agent_manager_ary[$i][0]['conv_min'];
	$mg_bottom_conv_min[$i] =$agent_manager_ary[$i][0]['conv_min'];
	$mg_top_ratio[$i]=$agent_manager_ary[$i][0]['ratio'];
	$mg_bottom_ratio[$i]=$agent_manager_ary[$i][0]['ratio'];
	for ($i_mg=0;$i_mg<$mg_agent_cnt[$i];$i_mg++)
	{
		if ($agent_manager_ary[$i][$i_mg]['duration']!=null and $mg_top_duration[$i_mg] < $agent_manager_ary[$i][$i_mg]['duration'])
			$mg_top_duration[$i_mg] = $agent_manager_ary[$i][$i_mg]['duration'];
		if ($agent_manager_ary[$i][$i_mg]['conv_min']!=null and $mg_top_conv_min[$i_mg] < $agent_manager_ary[$i][$i_mg]['conv_min'])
			$mg_top_conv_min[$i_mg] = $agent_manager_ary[$i][$i_mg]['conv_min'];
		if ($agent_manager_ary[$i][$i_mg]['ratio']!=null and $mg_top_ratio[$i_mg] < $agent_manager_ary[$i][$i_mg]['ratio'])
			$mg_top_ratio[$i_mg] = $agent_manager_ary[$i][$i_mg]['ratio'];
		
		if ($agent_manager_ary[$i][$i_mg]['duration']!=null and $mg_bottom_duration[$i_mg] > $agent_manager_ary[$i][$i_mg]['duration'])
			$mg_bottom_duration[$i_mg] = $agent_manager_ary[$i][$i_mg]['duration'];
		if ($agent_manager_ary[$i][$i_mg]['conv_min']!=null and $mg_bottom_conv_min[$i_mg] > $agent_manager_ary[$i][$i_mg]['conv_min'])
			$mg_bottom_conv_min[$i_mg] = $agent_manager_ary[$i][$i_mg]['conv_min'];
		if ($agent_manager_ary[$i][$i_mg]['ratio']!=null and $mg_bottom_ratio[$i_mg] > $agent_manager_ary[$i][$i_mg]['ratio'])
			$mg_bottom_ratio[$i_mg] = $agent_manager_ary[$i][$i_mg]['ratio'];		
	}
	...
	$mg_src_cnt[$i]=0;
	$sql_sel = sprintf("select lead_src,count(*) as source_cnt from customer_info,(select user_id from admin_user where owner='%s') as selected_user_id where customer_info.agent = selected_user_id.user_id group by lead_src order by source_cnt desc",$agent_ary[$i]['manager']);
	$sql_res = mysql_query($sql_sel) or die(mysql_error());
	while($sql_rec = mysql_fetch_assoc($sql_res))
	{
		if (trim($sql_rec['lead_src']) != "")
		{
			$mg_best_src_ary[$i][$mg_src_cnt[$i]]['lead_src']	= $sql_rec['lead_src'];
			$mg_src_cnt[$i]++;		
		}
	}
	$mg_max_row_cnt[$i]=max($mg_cd_cnt[$i],$mg_opp_cnt[$i],$mg_src_cnt[$i]);
...
$cd_cnt=0;
$sql_sel = sprintf("select sum(appr_lim) as appr_sum_lim, cd_name from funding_info group by cd_name order by appr_sum_lim desc;");
$sql_res = mysql_query($sql_sel) or die(mysql_error());
while($sql_rec = mysql_fetch_assoc($sql_res))
{
...
$best_opp_ary = array();
$opp_cnt=0;
$sql_sel = sprintf("select sum(case when yes_no='yes' THEN 1 ELSE 0 END) as yes_nos,opportunity from opportunity_info group by opportunity order by yes_nos desc;");
$sql_res = mysql_query($sql_sel) or die(mysql_error());
while($sql_rec = mysql_fetch_assoc($sql_res))
{
	if (trim($sql_rec['opportunity']) != "" and $sql_rec['yes_nos'] >0)
...
$best_src_ary = array();
$src_cnt=0;
$sql_sel = sprintf("select lead_src,count(*) as source_cnt from customer_info group by lead_src order by source_cnt desc;");
$sql_res = mysql_query($sql_sel) or die(mysql_error());
while($sql_rec = mysql_fetch_assoc($sql_res))
{
	if (trim($sql_rec['lead_src']) != "")
    ...
    <body>
		...
		<div class="container" style="margin:1px;padding:0px;width:99%">
			...
			<div class="row my-row">				
				<center><h1 style="margin-top:0px">Admin Report</h1></center>				
			</div>	
			<br>
			<!-- report tab panel -->
   			<div class="row my-row">
						...
						<form name="default_emplate" id="default_emplate" method="post" enctype="multipart/form-data">
                            ...
                            <div class="tab-content">
                               	<div class="tab-pane fade in active" id="report1_tab">
									...
									<div class="panel panel-primary" id="admin_panel">
										<div class="panel-heading">
								         	<h4 class="panel-title">
										    	<a data-toggle="collapse" data-target="#collapse_admin" href="#collapse_admin" >Admin</a>
										    </h4>
								      	</div>
								      	<div id="collapse_admin" class="panel-collapse collapse in">
														...
														<table class="table table-striped" style="margin:5px" width="100%">
										    				<thead>
																...
																<?php
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_ary[$i]['manager']) and $agent_ary[$i]['manager']!="")
																		{
																		?>
																			<th><a data-toggle="collapse" data-target="#collapse_manager<?php echo $i+1;?>" href="#collapse_manager<?php echo $i+1;?>" ><?php echo $agent_ary[$i]['manager'];?></a></th>
																		<?php	
																		}else
																		{															
																		?>
								...
								<?php 
									for ($i_mg=0;$i_mg<$mg_cnt;$i_mg++)
									{
									?>
									<div class="panel panel-info">
										<div class="panel-heading">
								         	<h4 class="panel-title">
										    	<a data-toggle="collapse" data-target="#collapse_manager<?php echo $i_mg+1;?>" href="#collapse_manager<?php echo $i_mg+1;?>" ><?php echo $agent_ary[$i_mg]['manager'];?></a>
										    </h4>
										</div>
										<div id="collapse_manager<?php echo $i_mg+1;?>" class="panel-collapse collapse">
											<div class="panel-body">
                                				<div class="row my-row">
													<div class="table-responsive" style="margin:2px;padding:2px;width:99%">
														<table class="table table-striped" style="margin:5px" width="100%">
										    				<thead>
																...
																<?php
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['agent']) and $agent_manager_ary[$i_mg][$i]['agent']!="")
																		{
																		?>
																			<th  ><?php echo $agent_manager_ary[$i_mg][$i]['agent'];?></th>
																		<?php	
																		}else
																		{															
																		?>
															...
															<tbody>
																...
																<tr >
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['calls']) and $agent_manager_ary[$i_mg][$i]['calls']!="")
																		{
																		?>
																			<td  style="width:8%"><?php echo $agent_manager_ary[$i_mg][$i]['calls'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['email']) and $agent_manager_ary[$i_mg][$i]['email']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['email'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['sms']) and $agent_manager_ary[$i_mg][$i]['sms']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['sms'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['followUpCount']) and $agent_manager_ary[$i_mg][$i]['followUpCount']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['followUpCount'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['past_due']) and $agent_manager_ary[$i_mg][$i]['past_due']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['past_due'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['delinquent']) and $agent_manager_ary[$i_mg][$i]['delinquent']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['delinquent'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['no_follow_up_date']) and $agent_manager_ary[$i_mg][$i]['no_follow_up_date']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['no_follow_up_date'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['newLeads']) and $agent_manager_ary[$i_mg][$i]['newLeads']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['newLeads'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['opened_emails']) and $agent_manager_ary[$i_mg][$i]['opened_emails']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['opened_emails'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['clickthroughs']) and $agent_manager_ary[$i_mg][$i]['clickthroughs']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['clickthroughs'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['retryLeads']) and $agent_manager_ary[$i_mg][$i]['retryLeads']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['retryLeads'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['hotLeads']) and $agent_manager_ary[$i_mg][$i]['hotLeads']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['hotLeads'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['warmLeads']) and $agent_manager_ary[$i_mg][$i]['warmLeads']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['warmLeads'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['credit_checks']) and $agent_manager_ary[$i_mg][$i]['credit_checks']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['credit_checks'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['credit_repairs']) and $agent_manager_ary[$i_mg][$i]['credit_repairs']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['credit_repairs'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['credit_ready']) and $agent_manager_ary[$i_mg][$i]['credit_ready']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['credit_ready'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['other_opportunity']) and $agent_manager_ary[$i_mg][$i]['other_opportunity']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['other_opportunity'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['pre_approveds']) and $agent_manager_ary[$i_mg][$i]['pre_approveds']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['pre_approveds'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['doc_sents']) and $agent_manager_ary[$i_mg][$i]['doc_sents']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['doc_sents'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['pending_fundings']) and $agent_manager_ary[$i_mg][$i]['pending_fundings']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['pending_fundings'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['fundedLeads']) and $agent_manager_ary[$i_mg][$i]['fundedLeads']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['fundedLeads'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['fee_pending']) and $agent_manager_ary[$i_mg][$i]['fee_pending']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['fee_pending'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['thirty_day_funding']) and $agent_manager_ary[$i_mg][$i]['thirty_day_funding']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['thirty_day_funding'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['sixty_day_funding']) and $agent_manager_ary[$i_mg][$i]['sixty_day_funding']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['sixty_day_funding'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['sixty_ninety_day_fundings']) and $agent_manager_ary[$i_mg][$i]['sixty_ninety_day_fundings']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['sixty_ninety_day_fundings'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['clients']) and $agent_manager_ary[$i_mg][$i]['clients']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['clients'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['duration']) and $agent_manager_ary[$i_mg][$i]['duration']!="")
																		{
																		?>
																			<td  <?php 
																			if ($agent_manager_ary[$i_mg][$i]['duration']==$mg_top_duration[$i_mg]) 
																				echo "style='color:green'"; 
																			else if ($agent_manager_ary[$i_mg][$i]['duration']==$mg_bottom_duration[$i_mg]) 
																				echo "style='color:red'"; 
																			?>> <?php echo (int)($agent_manager_ary[$i_mg][$i]['duration']/3600).':'.(int)(($agent_manager_ary[$i_mg][$i]['duration']%3600)/60).':'.(int)($agent_manager_ary[$i_mg][$i]['duration']%60);?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['conv_min']) and $agent_manager_ary[$i_mg][$i]['conv_min']!="")
																		{
																		?>
																			<td  <?php 
																			if ($agent_manager_ary[$i_mg][$i]['conv_min']==$mg_top_conv_min[$i_mg]) 
																				echo "style='color:green'"; 
																			else if ($agent_manager_ary[$i_mg][$i]['conv_min']==$mg_bottom_conv_min[$i_mg]) 
																				echo "style='color:red'"; 
																			?>><?php echo $agent_manager_ary[$i_mg][$i]['conv_min'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['call_made']) and isset($agent_manager_ary[$i_mg][$i]['call_conn']) and ($agent_manager_ary[$i_mg][$i]['call_made']!=""  or $agent_manager_ary[$i_mg][$i]['call_conn']!=""))
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['call_made'].'/'.$agent_manager_ary[$i_mg][$i]['call_conn'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['sms_sent'])  and isset($agent_manager_ary[$i_mg][$i]['sms_recv']) and ($agent_manager_ary[$i_mg][$i]['sms_sent']!="" or $agent_manager_ary[$i_mg][$i]['sms_recv']!=""))
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['sms_sent'].'/'.$agent_manager_ary[$i_mg][$i]['sms_recv'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['eml_sent']) and isset($agent_manager_ary[$i_mg][$i]['eml_recv']) and ($agent_manager_ary[$i_mg][$i]['eml_sent']!="" or $agent_manager_ary[$i_mg][$i]['eml_recv']!=""))
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['eml_sent'].'/'.$agent_manager_ary[$i_mg][$i]['eml_recv'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['ratio']) and $agent_manager_ary[$i_mg][$i]['ratio']!="")
																		{
																		?>
																			<td  <?php 
																			if ($agent_manager_ary[$i_mg][$i]['ratio']== $mg_top_ratio[$i_mg]) 
																				echo "style='color:green'"; 
																			else if ($agent_manager_ary[$i_mg][$i]['ratio']== $mg_bottom_ratio[$i_mg]) 
																				echo "style='color:red'"; 
																			?> ><?php echo $agent_manager_ary[$i_mg][$i]['ratio'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	for ($i=0;$i<10;$i++)
																	{
																		if (isset($agent_manager_ary[$i_mg][$i]['revenue']) and $agent_manager_ary[$i_mg][$i]['revenue']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg][$i]['revenue'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	...
																	<?php
																		...
																		if (isset($agent_manager_ary[$i_mg]['total']) and $agent_manager_ary[$i_mg]['total']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg]['total'];?></td>
																		<?php	
																		}else
																		{															
																		?>
																...
																<tr>
																	<td style="width:15%">Average</td>
																	<?php														
																		if (isset($agent_manager_ary[$i_mg]['average']) and $agent_manager_ary[$i_mg]['average']!="")
																		{
																		?>
																			<td  ><?php echo $agent_manager_ary[$i_mg]['average'];?></td>
																		<?php	
																		}else
																		{															
																		?>
								...
								<div class="tab-pane fade" id="report2_tab">
                            		<!--<h3>Report2</h3>-->
                            		<h5></h5>
                            		<div class="panel panel-primary" id="admin_panel_report2">
										<div class="panel-heading">
								         	<h4 class="panel-title">
										    	<a data-toggle="collapse" data-target="#collapse_admin_report2" href="#collapse_admin_report2" >Admin</a>
										    </h4>
								      	</div>
								      	<div id="collapse_admin_report2" class="panel-collapse collapse in">
											<div class="panel-body">
			                            		<div class="row my-row">
								...
								<?php 
									...
									{
									?>
									<div class="panel panel-info" id="admin_panel">
										<div class="panel-heading">
								         	<h4 class="panel-title">
										    	<a data-toggle="collapse" data-target="#collapse_admin_report2_<?php echo $i_mg+1;?>" href="#collapse_admin_report2_<?php echo $i_mg+1;?>" ><?php echo $agent_ary[$i_mg]['manager'];?></a>
										    </h4>
								      	</div>
								      	<div id="collapse_admin_report2_<?php echo $i_mg+1;?>" class="panel-collapse collapse">
											<div class="panel-body">
			                            		<div class="row my-row">
													<div class="table-responsive" style="margin:1px;padding:1px;">
														<table class="table table-striped " style="margin:10px" width="100%">
															...
															<tbody>															
																<?php		
																for ($i=0;$i<$mg_max_row_cnt[$i_mg];$i++)												
																{
																?>
																	<tr>
																		<!-- Highest Approved Limit Cards -->
																		<?php	
																		if ($i<$mg_cd_cnt[$i_mg] and ($mg_approved_card_ary[$i_mg][$i]) and $mg_approved_card_ary[$i_mg][$i]['cd_name']!="")
																		{
																		?>
																			<td><?php echo $mg_approved_card_ary[$i_mg][$i]['cd_name'];?></td>
																		<?php	
																		}else
																		{
																		?>
																		...
																		<!-- Best Opportunity -->
																		<?php	
																		if ($i<$mg_opp_cnt[$i_mg] and ($mg_best_opp_ary[$i_mg][$i]) and $mg_best_opp_ary[$i_mg][$i]['opportunity']!="")
																		{
																		?>
																			<td><?php echo $mg_best_opp_ary[$i_mg][$i]['opportunity'];?></td>
																		<?php	
																		}else
																		{
																		?>
																		...
																		<!-- Best Source -->
																		<?php	
																		if ($i<$mg_src_cnt[$i_mg] and ($mg_best_src_ary[$i_mg][$i]) and $mg_best_src_ary[$i_mg][$i]['lead_src']!="")
																		{
																		?>
																			<td><?php echo $mg_best_src_ary[$i_mg][$i]['lead_src'];?></td>
																		<?php	
																		}else
																		{
																		?>
