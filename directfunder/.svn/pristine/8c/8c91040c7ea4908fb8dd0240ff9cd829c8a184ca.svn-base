<?php
/****-------------------------------------------------------------------**************************	
		Purpose 	: 	Change password after agent login
...
if($_POST['Submit'] == "Change Password")
{
	...
	elseif($_POST['newpw'] != $_POST['rpw'])
	{
		$msg="The passwords you entered did not match.";
	}
	else
	{
		$sql_sel="select password from admin_user where user_id = '".$_SESSION['user_login']."'";
		$res=mysql_query($sql_sel) or die(mysql_error()."11");
		$res_rec=mysql_fetch_assoc($res);
		$old_pwd = $res_rec['password'];
		if ($old_pwd != $_POST['oldpw'])
		{
			$msg="The current password is wrong.";
		}else
		{
			$sql_change = sprintf("update admin_user set password='%s' where user_id ='%s'",$_POST['newpw'],$_SESSION['user_login']);
			mysql_query($sql_change) or die(mysql_error() . "go select error");			
		...
		<div class="container">
		    ...
		    <div class='row'>
		    	<div class='col-sm-4 col-md-offset-4'>
		          <form name="frmSearch" class="form-horizontal" method="post" action="passchange.php">
		          	<div class='form-row'>
		              <div class='col-xs-12 form-group' style="margin-left:0px;margin-right:0px">
		                <label class='control-label'>Current password</label>
		                <input autocomplete='off' class='form-control' type="password" name="oldpw" id="oldpw" size='4'>
		              </div>
		            </div>
		            <div class='form-row'>
		              <div class='col-xs-12 form-group' style="margin-left:0px;margin-right:0px">
		                <label class='control-label'>New password</label>
		                <input autocomplete='off' type="password" name="newpw" id="newpw" class='form-control' size='20' >
		              </div>
		            </div>
		            <div class='form-row'>
		              <div class='col-xs-12 form-group' style="margin-left:0px;margin-right:0px">
		                <label class='control-label'>Confirm password</label>
		                <input autocomplete='off' type="password" name="rpw" id="rpw" class='form-control' size='20'>
		              </div>
		            </div>    
		           
		            <div class='form-row'>
		              <div class='col-xs-12 form-group' style="margin-left:0px;margin-right:0px">
		                <button class='form-control btn btn-primary submit-button' type='submit' name="Submit" value="Change Password">Change password</button>                
		              </div>
		            </div>
		          </form>
		        </div>
