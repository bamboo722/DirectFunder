...
/* Bulk SMS */
if ($_POST['Submit'] == 'btnSmsBulkSubmit')
{
	//$_POST['btnSmsBulkSubmit'] ="";
	$file_nm = sprintf("bulk/tmp_sms_%d.txt",time());

	
	// delete all files in bulk Directory
	$dir = "bulk";
	$dirs = dir($dir);
	...
	// create new bulk temp file
	$myfile = fopen($file_nm, "w");
	$sms_body = $_POST['bulk_msg_body'];
	$sms_sal = $_POST['bulk_msg_sal'];
	$to_cnt=0;
	$customers_ary= explode(",", utf8_decode(rawurldecode($_POST['sel_customers'])));
	
	for ($i=0;$i<count($customers_ary); $i++)
	{
		$str = $customers_ary[$i];
		$str_ary = explode(";", $str);
		$phone_ary = explode(":", $str_ary[1]);
		for ($j=0;$j<count($phone_ary); $j++)
		{
			if ($phone_ary[$j]!="")
			{
				$phone_str = $phone_ary[$j]."\n";
				fwrite($myfile, $phone_str);									
				$to_cnt++;
			}			
		}					
	}	
    ...
    echo "window.open('sendbulksms.php?file=".$file_nm."&body=".$sms_body."&sal=".$sms_sal."&to_cnt=".$to_cnt."', width=330,height=330,left=100, top=25)";
    echo "</script>";
}

/* Bulk Email */
if ($_POST['Submit'] == 'btnEmailBulkSubmit')
{
	//$_POST['btnSmsBulkSubmit'] ="";
	$file_nm = sprintf("bulk/tmp_eml_%d.txt",time());
...
/*	$dir = "bulk";
	...
	// create new bulk temp file
	$myfile = fopen($file_nm, "w");
	$bulk_eml_subj = rawurlencode($_POST['bulk_email_subj']);
	$bulk_email_sal = $_POST['bulk_email_sal'];
	$bulk_email_body = rawurlencode($_POST['bulk_email_body']);
	...
	for ($i=0;$i<count($customers_ary); $i++)
	{
		$str = $customers_ary[$i];
		$str_ary = explode(";", $str);
		$email_ary = explode(":", $str_ary[0]);
		for ($j=0;$j<count($email_ary); $j++)
		{
			if ($email_ary[$j]!="")
			{
				$email_str = $email_ary[$j]."\n";
	...
	echo "<script type='text/javascript'>";
    echo "window.open('sendbulkemail.php?file=".$file_nm."&body=".$bulk_email_body."&sal=".$bulk_email_sal."&subj=".$bulk_eml_subj."&to_cnt=".$to_cnt."', width=330,height=330,left=100, top=25)";
    echo "</script>";
...
if ($_SESSION['sms']==-1)
{
	$sql_click_sms_ary['sms'] = 'getting...';
	$sql_click_call_ary['calls'] = 'getting...';
	$sql_click_email_ary['email'] = 'getting...';
}else
{
    ...
    <head>
		...
		<script type="text/javascript">
			...
			*/ 
			...
			// get notification if email you sent is openend 
		    ...
		    function submitForm()
            {
           	...
           	$(document).ready(function () {
				...
				/* get notification of call, email, sms */	       	
				function startTime_News() {
				   console.log("startTime_News");
				   $.ajax({
						type:"POST",
						dataType : "json",
				...
				/* get call, email, sms status during agent was logged out */				
				function getIntervalNews() {
				   console.log("getIntervalNews");
				   $.ajax({
						type:"POST",
						dataType : "json",
						url : "getIntervalNews.php",						
						success : function(res){
							console.log(res);
							//alert(res.status);
							if (res.status == "Success")
				...
				*/
			    ...
			    // bulk
			    $('#bulk_characterLeft').text('160 characters left');
			    $('#bulk_msg_body').keyup(function () {
			        var max = 160;
			        var len = $(this).val().length;
			        if (len >= max) {
			            $('#bulk_characterLeft').text('You have reached the limit');
			           
			            $('#btnSmsBulkSubmit').addClass('disabled');            
			        } 
			        else {
			            var ch = max - len;
			            $('#bulk_characterLeft').text(ch + ' characters left');
			            $('#btnSmsBulkSubmit').removeClass('disabled');
			           
			        }
			    });   
				$("#send_sms_btn").on("click", function() {	
					
					console.log('[send_sms_btn]');
					...
					document.getElementById("sel_customers").value = encodeURIComponent(arrchecked.toString());
					console.log(document.getElementById("sel_customers").value);
				  	$("#dialog_bulk_sms").modal();							
				});				
				
				$("#send_sms_btn_mobile").on("click", function() {					  
				 	console.log('[send_sms_btn_mobile]');
					var chk_ary = document.getElementsByName('sel_customer');
					console.log(chk_ary);
					var arrchecked = [];
					for (var i=0;i<chk_ary.length;i++)
			...
			function previewBulkSMS()
       		{
       			console.log("[PreviewBulkSMS]");      			
			
				var data = {					
					"sms_to":"Bulk Message",
					"sms_sal":document.getElementById('bulk_msg_sal').value,
					"sms_body":document.getElementById('bulk_msg_body').value						
				};
				console.log(data);
			        ...
			        success: function ( res ) {
			        	...
			        	if (res.status == "Success")
			        	{
			        		if (res.sms_body != "")
			        		{
								...
								document.getElementById("bulk_sms_preview_div").innerHTML =res.sms_body;								
          						$('#dialog_preview_bulk_sms').modal();            					
							}							
						}							
			        }
			    });				   
			...
			function Select_all_records()
			{
				...
				//var tmp = document.getElementsByName('sel_customers[]');	
				//console.log(tmp);
    ...
    <body onmousedown="MyMouseDown()">
		...
		<div class="container" style="margin:1px;width:99%">
  			...
  			<div id="my-main-content">
						...
						<div id="tab-container" class="head-box-layer" style="line-height : 1.2;">
	                        ...
	                        <div>  
	                            <table class="head-box mobile-info-box" style="background-color: #3e75b4;">
	                                ...
	                                <tr align="center">
	                                     <td ><div id="dur_div"><?php echo '00:00:00';?></div></td>
	                                </tr>                                     
	                            </table>
	                        </div>
	                        <div> 
        		...
        		<br>
        		<div class="row">
					<div class="col-md-offset-2 col-md-8 col-sm-offset-3 col-sm-6 col-xs-11">
						<div class="input-group" id="adv-search" style="display: inline">
							<div class="col-md-10 col-sm-10 col-xs-10" style="padding-right:0px">
			              	  	<input type="text" class="form-control my-form-control" id="search_value" style="height:34px !important" placeholder="Search for" value="<?php
									if (isset($_POST['val'])) {
	                                    echo $_POST['val'];
	                                }?>"/>
	                          </div>
			                  <div class="input-group-btn col-md-2 col-sm-2 col-xs-2" style="padding-left:0px;">
			    ...
			    <div class="row">
					<div class="col-md-12 ">
						<form class="form-horizontal" method="post" action="searchbuyer.php" >
							<table class="table" style="border:0px;margin:0px">
								<thead>
									<tr >
										...
										<!-- for selected customers information -->
										<input type="hidden" id="sel_customers"  name="sel_customers" value='0'/>	
										<!--<td class="col-md-9" align="right" style="padding:8px 0px 0px 0px"><button type="button" class="btn btn-success btn-sm " id="send_sms_btn">SMS<span id="sms_news_span" style="visibility:hidden;color:white !important;background-color:red !important" class="badge"></span></button>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-success btn-sm " id="send_eml_btn">Email<span id="email_new_span" style="visibility:hidden;color:white !important;background-color:red !important" class="badge"></span></button></td>-->
										<td class="col-md-9" align="right" style="padding:8px 0px 0px 0px"><button type="button" class="btn btn-success btn-sm " name="send_sms_btn" value="send_sms_btn" id="send_sms_btn">SMS<span id="sms_news_span" style="visibility:hidden;color:white !important;background-color:red !important" class="badge"></span></button>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-success btn-sm " id="send_eml_btn">Email<span id="email_new_span" style="visibility:hidden;color:white !important;background-color:red !important" class="badge"></span></button></td>
										...
										<td class="col-md-1" style="padding:8px 0px 0px 0px" >
				                            <select class="form-control" onchange="ChangeDisplayCnt();" id="display_recs" name="display_recs">
					                                ...
					                                <option value="2000" <?php
					                                    if ($_POST['display_recs'] == "2000") {
					                                        echo 'selected';
					                                    }
					                                    ?>>2000</option>
					                                <option value="3000" <?php
					                                    if ($_POST['display_recs'] == "3000") {
					                                        echo 'selected';
					                                    }
					                                    ?>>3000</option>
					                                <option value="5000" <?php
					                                    if ($_POST['display_recs'] == "5000") {
					                                        echo 'selected';
					                                    }
					                                    ?>>5000</option>
					                                <option value="10000" <?php
					                                    if ($_POST['display_recs'] == "10000") {
					                                        echo 'selected';
					                                    }
					                                    ?>>10000</option>
					                                <option value="15000" <?php
					                                    if ($_POST['display_recs'] == "15000") {
					                                        echo 'selected';
					                                    }
					                                    ?>>15000</option>
					                                <option value="25000" <?php
					                                    if ($_POST['display_recs'] == "25000") {
					                                        echo 'selected';
					                                    }
					                                    ?>>25000</option>
					                                <option value="50000" <?php
					                                    if ($_POST['display_recs'] == "50000") {
					                                        echo 'selected';
					                                    }
					                                    ?>>50000</option>
		  					...
		  					<!-- sms bulk send modal dialog -->
							<div id="dialog_bulk_sms" class="modal fade" title="Send SMS" style="z-index:1000000005;display:none;">
									...
									<div class="modal-content">
										<div class="modal-header">
								      	...
								      	<div class="modal-body">						      		
									        <div class="form-group">
									        	<label for="bulk_msg_sal">Salutation</label>
									        	<input type="text" name="bulk_msg_sal" id="bulk_msg_sal"   value="Hello" class="form-control"/>
									        </div>
									        <div class="form-group">
									          	<label for="bulk_msg_body">Message</label>
									           	<textarea id="bulk_msg_body" name="bulk_msg_body" rows="7"  maxlength="160" class="form-control"></textarea>	
									           		<span class="help-block"><p id="bulk_characterLeft" class="help-block ">You have reached the limit</p></span>     			    
									        </div>			
								      	</div>
										<div class="modal-footer">
											<div class="col-xs-6">
								      			<button type="submit" id="btnSmsBulkSubmit"  name="Submit" value="btnSmsBulkSubmit" class="btn btn-default btn-success btn-block"><span class="glyphicon glyphicon-off"></span>&nbsp;Send</button>			        	 
								      		</div>
								      		<div class="col-xs-6">
								      			<button type="button" onclick="javascript:previewBulkSMS()" class="btn btn-default btn-success btn-block"><span class="glyphicon glyphicon-eye-open"></span>&nbsp;Preview</button>								      		
									      	</div>	
										</div>
									</div>
								</div>		 
							...
							<!-- sms bulk preview dialog -->
					        <div id="dialog_preview_bulk_sms" class="modal fade" style="z-index:1000000006;display:none;" title="" >
					        	<div class="modal-dialog modal-md">
									<div class="modal-content">
										<div class="modal-header">
									        <button type="button" class="close" data-dismiss="modal">&times;</button>
									        <h4 class="modal-title">Preview Bulk SMS</h4>				        
								      	</div>
								      	<div class="modal-body" style="overflow:auto" id="bulk_sms_preview_div">
								      		
								      	</div>	
								      	<div class="modal-footer">
								      		<center>			      			
								      			<div class="col-xs-offset-4 col-xs-4">
								      				<button type="button" onclick="javascript:sendBulkSMS()" class="btn btn-default btn-success btn-block"><span class="glyphicon glyphicon-off"></span>&nbsp;Send</button>
								      			</div>			      							      		
								      		</center>			      		
								      	</div>				     
									</div>
							...
							<!-- email bulk send modal dialog -->
							<div id="dialog_bulk_email" class="modal fade" style="z-index:1000000010;display:none;"  title="Send Email">
								<div class="modal-dialog modal-sm">
									<div class="modal-content">
										<div class="modal-header">
									        <button type="button" class="close" data-dismiss="modal">&times;</button>
								      	...
								      	<div class="modal-body">
								        	<form enctype="multipart/form-data" method="post" id="SendEmailUpload">
								        		<div class="form-group">
									            	<label for="bulk_email_subj">Subject</label>
									            	<input type="text" name="bulk_email_subj" id="bulk_email_subj" value="<?php echo $eml_subj;?>" class="form-control"/>				            	
									            </div>
												<div class="form-group">
									            	<label for="user_bulk_email_attach_file_name">Attach File</label>
									            	<input name="user_bulk_email_attach_file_name" type="file"  value="<?php echo $eml_att;?>" class="form-control"/>
									            </div>
									            <div class="form-group">
									            	<label for="bulk_email_sal">Salutation</label>
									            	<input type="text" name="bulk_email_sal" id="bulk_email_sal"   value="Hello" class="form-control"/>
									            </div>
									            <div class="form-group">
									            	<label for="bulk_email_body">Content</label>
									            	<textarea id="bulk_email_body" name="bulk_email_body" rows="5" cols="20" class="form-control"><?php echo $eml_cont;?></textarea>				    
									            </div>							
											</form>
								      	</div>
								      	...
								      	<div class="modal-footer">
								      		<div class="row">
								      			<div class="col-xs-6">
								      				<button type="submit" id="btnEmailBulkSubmit"  name="Submit" value="btnEmailBulkSubmit" class="btn btn-default btn-success btn-block"><span class="glyphicon glyphicon-off"></span>&nbsp;Send</button>			        	 
								      			</div>
								      			<div class="col-xs-6">
								      				<button type="button" onclick="javascript:previewBulkEmail()" class="btn btn-default btn-success btn-block"><span class="glyphicon glyphicon-eye-open"></span>&nbsp;Preview</button>			        	 
								      			</div>
								      		</div>	
								      	</div>					
									</div>
					        ...
					        <div id="dialog_preview_bulk_email" class="modal fade" style="z-index:1000000011;display:none;" title="">
									...
									<div class="modal-content">
										<div class="modal-header">
								      	...
								      	<div class="modal-footer">
								      		<center>			      			
								      			<div class="col-xs-offset-4 col-xs-4">
								      				<button type="button" onclick="javascript:sendBulkEmail()" class="btn btn-default btn-success btn-block"><span class="glyphicon glyphicon-off"></span>&nbsp;Send</button>
								      			</div>			      							      		
								      		</center>			      		
								      	</div>				     
									</div>
					                        ...
					                        while ($seerec = mysql_fetch_assoc($result))
					                        {
			                                    ...
			                                    <tr>
			                                    	<td >
			                                    		...
			                                    		<!--<input type="checkbox"  name="sel_customers[]"  value="<?php echo $seerec['p_eml1'].':'.$seerec['p_eml2'].':'.$seerec['p2_eml'].':'.$seerec['p3_eml'].';'.$seerec['p_ph1'].':'.$seerec['p_ph2'].':'.$seerec['p2_ph1'].':'.$seerec['p2_ph2'].':'.$seerec['p3_ph1'].':'.$seerec['p3_ph2'].';'.$seerec['customer_id']; ?>">-->
			                                    		<input type="checkbox"  name="sel_customer" value="<?php echo $seerec['p_eml1'].':'.$seerec['p_eml2'].':'.$seerec['p2_eml'].':'.$seerec['p3_eml'].';'.$seerec['p_ph1'].':'.$seerec['p_ph2'].':'.$seerec['p2_ph1'].':'.$seerec['p2_ph2'].':'.$seerec['p3_ph1'].':'.$seerec['p3_ph2'].';'.$seerec['customer_id']; ?>">
		...
		<div id="dialog_email" class="modal fade" style="z-index:1000000000;display:none;"  title="Send Email">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					<div class="modal-header">
				        <button type="button" class="close" data-dismiss="modal">&times;</button>
			      	...
			      	<div class="modal-body">
			        	<form enctype="multipart/form-data" method="post" id="SendEmailUpload">
				            ...
				            <div class="form-group">
				            	<label for="email_sal">Salutation</label>
