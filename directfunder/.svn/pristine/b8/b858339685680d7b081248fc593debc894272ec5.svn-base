		  ...
		  <table width="99%" border="0" cellspacing="0" cellpadding="0" class="table_disp_out">
				...
				?>
                      ...
                      <tr> 
                        <td align="center" class="midbody_lebel"> <table width="66%">
                            <tr> 
                              <td width="74%"><input type="text" name="mailField" size="50" value="<?php echo $customer_mail; ?>"></td>
			...
			else
			{
				$v_customer=explode(",", $_POST['mailField']);
						...
						if ($_POST['Send']=="Send")
						{
								if($_POST['source']=="Manual")
								{
									$mailsubject = 'Alpine yogurt & soft serve machine'; 
								}
								else if($_POST['source']=="Financing")
								{
									$mailsubject = 'Financing application for Alpine yogurt & soft serve machines'; 
								}
								else if($_POST['source']=="Quotes")
								{
									...
									echo $mailsubject;
									echo 'mail id ='.$v_customer[$m].' msage body='.$msg1.' subject body='.$mailsubject.'from='.$Fromname;
									while ($v_customer[$m]!="")
									{
										echo 'mail id ='.$v_customer[$m].' msage body='.$msg1.' subject body='.$mailsubject.'from='.$Fromname;
										...
										values
										 (
										 '".$v_customer[$m]."',
										 'Alpine soft serve yogurt machines',
										 '".$_POST['rte1']."',
										  sysdate(),
										 'Duc Nguyen',
										 'duc@cenpaco.com'
