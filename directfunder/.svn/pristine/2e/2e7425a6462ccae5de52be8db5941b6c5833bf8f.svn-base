...
if ($_SESSION['user_group'] == 'Manager')
{
	...
	$agent_addr = $_SESSION['google_acc_nm'];
	
	$filterAgent = " and ( ((from_address like ('".$agent_addr."')) or (mail_rcvr like ('".$agent_addr."'))) ";
	while($sql_rec_agents = mysql_fetch_assoc($sql_res_agents))
	{
		$agent_addr = $sql_rec_agents['google_acc_nm'];
		if ($sql_rec_agents['user_id']!="")	
			$filterAgent .= " or ((from_address like ('".$agent_addr."')) or (mail_rcvr like ('".$agent_addr."'))) ";
	}
	$filterAgent .= ")";
}else if ($_SESSION['user_group'] == 'Agent')
{
	$filterAgent = " and ((from_address like ('".$_SESSION['google_acc_nm']."')) or (mail_rcvr like ('".$_SESSION['google_acc_nm']."'))) ";
}
$sql_click_email = "SELECT * from mail_log_info where 1 $filterAgent order by send_dt desc";

/*if ($_SESSION['user_group'] == 'Admin')
	$sql_click_email = "SELECT *  from mail_log_info order by send_dt desc";
else	
	$sql_click_email = "SELECT *  from mail_log_info where ((from_address like ('".$_SESSION['google_acc_nm']."')) or (mail_rcvr like ('".$_SESSION['google_acc_nm']."'))) order by send_dt desc ";*/
$seeres = mysql_query($sql_click_email) or die(mysql_error());
...
<html>
	...
	<body>
	<form name="frmSearch" method="post" action="getHistory_all_emails.php">
		<div class="container">
			<h1>&nbsp;</h1>
			<div class="row">
