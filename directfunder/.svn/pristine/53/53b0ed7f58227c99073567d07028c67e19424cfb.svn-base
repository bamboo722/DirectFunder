...
if ($_SESSION['user_group'] == 'Admin')
	$sql_click_sms = "SELECT * from sms_log_info";
else	
	$sql_click_sms = "SELECT * from sms_log_info where ((from_phone like ('".$phone."')) or (to_phone like ('".$phone."'))) ";
$seeres = mysql_query($sql_click_sms) or die(mysql_error());
...
<html>
	...
	<body>
		...
		<form name="frmSearch" method="post" action="getHistory_all_sms.php">
			<div class="container">
				<h1>&nbsp;</h1>
				<div class="row">
				      		...
				      		<div class="table-responsive" style="height:600px; overflow:auto; overflow-y:scroll;">         
				    			<table class="table table-striped" >  
									...
									<?php 
									  	  ...
									  	  {					  	  	
									  	   	 if ($response['sms_time'][$i] > $_SESSION['last_logout']) {
												if ($response['sms_flag'][$i] == 1) {
													$style="style='font-size:16px;min-height:50px;font-weight:bold;color:#0c00ea;'";
				                            	}else{
													$style="style='font-size:16x;min-height:50px;color:#0c00ea;'";	                                    
				                                }	
				                            }else
