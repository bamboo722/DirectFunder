...
$sql_sel = sprintf("select mail_id,mail_rcvr,from_address from mail_log_info where mail_id is not null and is_opened='%d' and mail_stat='%d'",0,0);
$sql_res = mysql_query($sql_sel) or die(mysql_error());
while ($sql_rec = mysql_fetch_assoc($sql_res))
{
	$id = $sql_rec['mail_id']; 
	$from = $sql_rec['from_address']; 
	$to = $sql_rec['mail_rcvr'];
	$checkid = "Id:" . $id;
	$fh = fopen("http://www.directfunder.com/prospects-mobile/email.txt", "r");
